...
module.exports = async function (source, map) {
  ...
  const filePath = this.resourcePath.match(/(.*)((\.vue$)|(\.js$)|(\.ts$))/)
      ...
      for(let replacer of replacers){
        ...
        if (replacer.hash) {
          ...
          for(let lang of options.languages) {
            data[lang.key][replacer.hash] = lang.translator ? 
            await lang.translator(replacer.origin) : 
            defaultTranslator(replacer.origin)
          }
        }
      }
      // write file with cacheTime (to handle webpack rebuild bug)
      utils.writeJsonToFile(data, filePath[1] + '.locales.json', ignoreDeprecatedMarkLangs)
    } else {
